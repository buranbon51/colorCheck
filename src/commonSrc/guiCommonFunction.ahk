...
guf_showGuiAssign(GuiNum, pOrder="_"){
	if(pOrder == "_"){
		Gui, %GuiNum%:Show
	} else {
		Gui, %GuiNum%:Show, %pOrder%
	}
}
...
guf_GuiControl(pControl, GuiNum, pOrder, param="_"){
	if(param == "_"){
		GuiControl, %GuiNum%:%pOrder% ,%pControl%
	} else {
		GuiControl, %GuiNum%:%pOrder% ,%pControl%, %param%
	}
}
...
guf_Gui(pOrder, GuiNum, value1="_", value2="_", value3="_"){
	if(value3 != "_"){
		Gui, %GuiNum%:%pOrder% ,%value1%, %value2%, %value3%
	} else if(value2 != "_"){
		Gui, %GuiNum%:%pOrder% ,%value1%, %value2%
	} else if(value1 != "_"){
		Gui, %GuiNum%:%pOrder% ,%value1%
	} else {
		Gui, %GuiNum%:%pOrder%
	}
}

guf_GuiControlGet(pControl, GuiNum, pOrder="_", value="_"){
	if(value != "_"){
		GuiControlGet, OutputVar, %GuiNum%:%pOrder%, %pControl%, %value%
	} else if(pOrder != "_"){
		GuiControlGet, OutputVar, %GuiNum%:%pOrder%, %pControl%
	} else {
		GuiControlGet, %pControl%, %GuiNum%:
		OutputVar := %pControl%
	}
...
; 表示・非表示をトグルにする
guf_showHideEnyGui(examplControl, GuiNum, pOrder="_"){
...
guf_showHideGuiToggle(winTitle, GuiNum, pOrder="_"){
	IfWinExist, %winTitle%
	{
		Gui, %GuiNum%:Hide
	} else {
